...
func (p ProfileMysqlInteractor) UpdateEducation(ctx context.Context, id string, education *profile.Education) error {
	...
	_, err := dbq.E(ctx, p.DbConn, query, nil, education.GetLevel().StringLevel(), education.GetName(), education.GetMajor(), education.GetStillEducation(), education.GetStartEducation(), education.GetEndEducation(), education.GetDescription(), education.GetUpdatedAt(), timeNow, id)
